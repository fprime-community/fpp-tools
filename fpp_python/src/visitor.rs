//! [`AstVisitor`]: the read-only AST traversal base class.
//!
...
//! trait is shallow by default — matching `ast.AstVisitor`, which is what Python
...
//! # Structure
//!
//! Only [`AstVisitor::visit`] and [`AstVisitor::generic_visit`] carry logic;
...
/// What [`AstVisitor::visit`] accepts: a single AST node, one translation unit,
...
impl AstVisitor {
    ...
    /// dispatch. `AstVisitor` is `frozen`, so the `PyRef` carries no borrow
